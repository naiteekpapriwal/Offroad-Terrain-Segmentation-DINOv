...
value_map = {
    ...
    100: 0,    # Trees -> 0
    200: 0,    # Lush Bushes -> 0
    300: 1,    # Dry Grass (The Path) -> 1
    500: 0,    # Dry Bushes -> 0
    550: 0,    # Ground Clutter -> 0
    700: 0,    # Logs -> 0
    800: 0,    # Rocks -> 0
    7100: 0,   # Landscape -> 0
    10000: 0   # Sky -> 0
...
def main():
    ...
    data_dir = r'C:\Users\hp\Downloads\Offroad_Segmentation_Scripts (1)\train'
    val_dir = r'C:\Users\hp\Downloads\Offroad_Segmentation_Scripts (1)\val'
    ...
    # Loss and optimizer
   # Define weights for all 10 classes in your value_map
    # [Background, Trees, Lush Bushes, Dry Grass, Dry Bushes, Clutter, Logs, Rocks, Landscape, Sky]
    weights = torch.tensor([1.0, 10.0]).to(device) # Handshake with the 2 classes
    loss_fct = torch.nn.CrossEntropyLoss(weight=weights)
    optimizer = torch.optim.Adam(classifier.parameters(), lr=lr) # Adam is faster than SGD
    ...
    for epoch in epoch_pbar:
        ...
        # Store history (This part is already in your code)
        history['val_iou'].append(val_iou) 

        # --- PASTE THE SCHEDULER STEP HERE ---
